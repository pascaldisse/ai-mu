...
/// V9V INPUT FIREFLY CLAMP — CLAMP PARITY (magic-crystal v9y task mandate,
/// 2026-07-25): ported from the v9body trainer's own
/// `examples/rdirect_train_v9k.rs::v9v_input_clamp_delta`/`apply_input_clamp`
/// (also `examples/rdirect_v9w2_triptych.rs`, ported verbatim there too).
/// v9y (the checkpoint this eye-test window loads) was trained with
/// `GAIA_V9V_INPUT_CLAMP=0.10` (non-zero `input_clamp_taps` every epoch,
/// scratch/v9y-train.log in v9body) — the live gather (`gather_v9` in
/// `rdirect_gather_split.wgsl`) did NOT implement this clamp before this fix
/// (verified: no `INPUT_CLAMP`/median/ceiling logic anywhere in the WGSL or
/// this crate). Per channel per 2×2 tap block (E's taps then D's taps,
/// `pixel_features_split`'s own tap-major/channel-minor layout): ceiling =
/// median-of-the-4-taps + delta (SAME demod-log domain the taps are already
/// encoded in, SAME divisor/albedo-demod math — `pixel_features_split` and
/// `gather_v9` share the identical `log_demod`/`log(max(d,0)+1.0)`
/// construction); any tap above the ceiling is pulled down. `delta<=0.0`
/// (default, matches the CPU trainer's own default) is a no-op — IRON LAW
/// byte-identical to the pre-clamp gather. Wired into `gather_v9` only (the
/// v9 eye-test window's actual live feature path — `FeatureGatherV9::encode`,
/// `rdirect_gather.rs`); the older `gather_split`/`gather_hist_split` entries
/// (v7-live-lane stages, not used by the v9 path) are left byte-untouched.
pub fn v9v_input_clamp_delta() -> f32 {
    std::env::var("GAIA_V9V_INPUT_CLAMP").ok().and_then(|v| v.parse().ok()).unwrap_or(0.0)
}
