...
struct HistU {
  ...
  params: vec4<f32>,
  // params2 = (normal_thresh, sky_reject, input_clamp_delta, pad) --
  // sky_reject is GAIA_V7_SKY_HISTORY=reject as 1.0/0.0 (see
  // rdirect.rs::sky_history_reject). input_clamp_delta is
  // GAIA_V9V_INPUT_CLAMP (see rdirect.rs::v9v_input_clamp_delta) -- ONLY
  // read by `gather_v9` below (the V9V input firefly clamp, task mandate);
  // `gather_hist_split` never reads params2.z, so `FeatureGatherHistSplit`
  // leaving it at 0.0 is inert, not a behavior change.
  params2: vec4<f32>,
};
...
// ─────────────────────────────────────────────────────────────────────────
// V9-WIRE ATOM (2026-07-21) — GPU-NATIVE GATHER for the v9 body's 41-in
// layout (39 `HIST_FEATURES_SPLIT`, byte-identical to `gather_hist_split`
// above, + 2 REAL screen-space motion-vector channels). Mirrors CPU
// `rdirect_train_v9c.rs::build_input_img`/`reproject_prev` (also ported to
// `rdirect_frame_wall.rs::build_input_img_unet`/`reproject_prev_unet`)
// bit-for-bit: the split base (idx 0-34, motion-vector SLOT inside the base
// itself stays zero — that field is dead in the v9 CPU reference too, see
// `pixel_features_split`'s own `Vec2::ZERO` call site) + history (idx
// 35-38, SAME reprojection/validity gate as Stage 2) + the REAL mv (idx
// 39-40) = `(fx - tx, fy - ty)` in fractional target pixels, computed from
// the SAME reprojection this entry already runs for history — and, per the
// CPU reference's own contract, only nonzero when `valid == 1.0` (mv is
// zeroed alongside prev_dl on any miss/reject, not just on `has_prev=false`).
// ADDITIVE: `gather_split`/`gather_hist_split` above stay byte-untouched.

const FEATURES_V9: u32 = 41u;

// V9V INPUT FIREFLY CLAMP (task mandate, clamp-parity fix 2026-07-25) --
// ported bit-for-bit (same domain, same tap-major/channel-minor layout,
// same median-of-4+delta ceiling) from CPU `rdirect_train_v9k.rs::
// apply_input_clamp` (mirrored in this crate's `rdirect.rs::
// v9v_input_clamp_delta` doc). `gather_v9` is the ONLY entry point that
// applies this -- `gather_split`/`gather_hist_split` above stay
// byte-untouched (v7-live-lane stages, not the v9 live path).

// Classic 5-comparator sorting network for 4 elements -> returns the
// median (average of the sorted middle two), matching CPU's
// `sorted.sort_by(...); 0.5*(sorted[1]+sorted[2])`.
fn median_of_4(v0: f32, v1: f32, v2: f32, v3: f32) -> f32 {
  var a = v0;
  var b = v1;
  var c = v2;
  var d = v3;
...
// `block` is one E-or-D tap block: [tap*3 + c] (tap 0..4, channel 0..3),
// SAME layout CPU's `apply_input_clamp` indexes via `block_start + tap*3 +
// c`. `delta<=0.0` is a no-op (IRON LAW, byte-identical to no clamp).
fn apply_input_clamp_block(block: ptr<function, array<f32, 12>>, delta: f32) {
  if (delta <= 0.0) { return; }
  for (var c = 0u; c < 3u; c = c + 1u) {
    let v0 = (*block)[0u * 3u + c];
    let v1 = (*block)[1u * 3u + c];
    let v2 = (*block)[2u * 3u + c];
    let v3 = (*block)[3u * 3u + c];
    let ceiling = median_of_4(v0, v1, v2, v3) + delta;
    if (v0 > ceiling) { (*block)[0u * 3u + c] = ceiling; }
    if (v1 > ceiling) { (*block)[1u * 3u + c] = ceiling; }
    if (v2 > ceiling) { (*block)[2u * 3u + c] = ceiling; }
    if (v3 > ceiling) { (*block)[3u * 3u + c] = ceiling; }
  }
}

@compute @workgroup_size(8, 8, 1)
fn gather_v9(@builtin(global_invocation_id) gid: vec3<u32>) {
  let tw = u.dims.z;
  let th = u.dims.w;
  if (gid.x >= tw || gid.y >= th) { return; }
  let tx = gid.x;
  ...
  let base = i * FEATURES_V9;
  var k = 0u;

  // V9V INPUT FIREFLY CLAMP: gather all 4 taps for this block BEFORE
  // writing (the median needs all 4), clamp, then write tap-major exactly
  // as before -- `feats` layout/order is byte-identical when clamp_delta
  // <= 0.0.
  let clamp_delta = hu.params2.z;
  var e_block: array<f32, 12>;
  for (var t = 0u; t < 4u; t = t + 1u) {
    let cell = accum_ed[2u * taps[t] + 0u];
    let rad = cell.xyz / max(cell.w, 1.0);
    let d = rad / divisor;
    e_block[t * 3u + 0u] = log(max(d.x, 0.0) + 1.0);
    e_block[t * 3u + 1u] = log(max(d.y, 0.0) + 1.0);
    e_block[t * 3u + 2u] = log(max(d.z, 0.0) + 1.0);
  }
  apply_input_clamp_block(&e_block, clamp_delta);
  for (var t = 0u; t < 4u; t = t + 1u) {
    feats[base + k + 0u] = e_block[t * 3u + 0u];
    feats[base + k + 1u] = e_block[t * 3u + 1u];
    feats[base + k + 2u] = e_block[t * 3u + 2u];
    k = k + 3u;
  }
  var d_block: array<f32, 12>;
  for (var t = 0u; t < 4u; t = t + 1u) {
    let cell = accum_ed[2u * taps[t] + 1u];
    let rad = cell.xyz / max(cell.w, 1.0);
    let d = rad / divisor;
    d_block[t * 3u + 0u] = log(max(d.x, 0.0) + 1.0);
    d_block[t * 3u + 1u] = log(max(d.y, 0.0) + 1.0);
    d_block[t * 3u + 2u] = log(max(d.z, 0.0) + 1.0);
  }
  apply_input_clamp_block(&d_block, clamp_delta);
  for (var t = 0u; t < 4u; t = t + 1u) {
    feats[base + k + 0u] = d_block[t * 3u + 0u];
    feats[base + k + 1u] = d_block[t * 3u + 1u];
    feats[base + k + 2u] = d_block[t * 3u + 2u];
    k = k + 3u;
  }
  feats[base + k + 0u] = dx;
  feats[base + k + 1u] = dy;
  ...
  // ── HISTORY (idx 35-38) + REAL MOTION VECTOR (idx 39-40) ──
  var prev_dl = vec3<f32>(0.0, 0.0, 0.0);
  var valid = 0.0;
  var mv = vec2<f32>(0.0, 0.0);
  let has_prev = hu.params.z;
  if (has_prev > 0.5) {
    let prev_w = u32(hu.params.x);
    let prev_h = u32(hu.params.y);
    ...
    if (rep.z > 0.5) {
      ...
      if (ok) {
        prev_dl = bilinear_prev_dl(fx, fy, prev_w, prev_h);
        valid = 1.0;
        // MOTION-BUG FORENSICS FIX (2026-07-24, Pascal's eye-test flicker):
        // v9t's ENTIRE training regime feeds this channel pair values from a
        // near-static camera only — the validator/bar-res-probe path
        // (`rdirect_train_v9k.rs::bar_res_probe`, mirrored in
        // `examples/rdirect_v9t_visual.rs`) reuses ONE FIXED camera pose
        // across all K recurrent-refinement steps (`let cam = val_cam;`, no
        // per-step pan at all — mv is EXACTLY zero every validator/settle()
        // call), and the train-time crop sequences (`render_pose_seq*`) only
        // ever pan `GAIA_V9_PANSTEP=0.004` RADIANS per step (v9t-train.log
        // line 16) — a few PIXELS of shift at most. The first conv layer's
        // weight columns for this channel pair therefore never saw (and so
        // never learned to handle) anything resembling a real player turn/
        // walk between frames (tens-to-hundreds of px) — confirmed by ordeal:
        // feeding this live wiring's REAL frame-to-frame mv reproduced
        // Pascal's reported flicker exactly (scratch/v9-motion-final-T*.png,
        // scratch/v9smooth-*.png — fine standing still, catastrophic
        // blue-flood + chromatic-fringe corruption the instant the camera
        // moves, worsening with sustained motion, partially recovering only
        // where reprojection itself rejects/resets). Zeroing this pair here
        // makes the live feature distribution match what v9t was ACTUALLY
        // trained on (near-zero mv, always) instead of feeding real motion
        // through effectively random/unlearned weights — the recurrent
        // history proper (`prev_dl`/`valid` above, channels 35-38) is
        // UNCHANGED and stays fully exercised (that IS v9t's trained
        // multi-pass denoise mechanism). Re-enable a real mv signal only
        // alongside a checkpoint retrained with actual inter-frame camera
        // deltas in its K-unroll (disclosed gap, not silently papered over).
        mv = vec2<f32>(0.0, 0.0);
      }
    }
  }
  ...
  feats[base + k + 2u] = prev_dl.z;
  feats[base + k + 3u] = valid;
  feats[base + k + 4u] = mv.x;
  feats[base + k + 5u] = mv.y;
}
