...
struct Uniform {
  ...
  temporal_flags: vec4<u32>, // history_valid (0/1), max_history frames, bitcast<f32> still_px (sub-pixel motion budget), bitcast<f32> GAIA_NEE_CLAMP (MAGIC CRYSTAL PART 2 firefly clamp, 0 = off)
};
...
// ── STAGE 1: EMISSIVE NEE — the light list ───────────────────────────────
// One emissive triangle, built host-side once per BVH build/merge (see
// `build_emissive_lights` in integrator.rs) — POWER-weighted (power_i =
// luminance(emission_i) * area_i, a standard proxy for radiant power) so a
// big bright orb is picked more often than a dim sliver, cutting variance
// vs. a uniform-by-count pick. `tri` indexes `tris` directly (the SAME
// leaf-order index space `hit.tri` already uses — no remapping). `area` is
// the triangle's world-space area (uniform-point-on-triangle pdf = 1/area).
// `pdf` is this light's own selection probability (power_i/total_power);
// `cdf` is the INCLUSIVE running sum of `pdf` in build order, for O(log N)
// binary-search sampling (`sample_light_index`) — the last entry is exactly
// 1.0 (clamped host-side against fp drift). A SEPARATE @group(2) so
// pipelines that never call the NEE-carrying radiance variants
// (`integrate_temporal`/`temporal_resolve`, already at the 8-storage-buffer/
// stage cap; `integrate_aov`, which never shades) never need this binding.
struct Light {
  tri: u32,
  area: f32,
  pdf: f32,
...
// ── STAGE 1: EMISSIVE NEE — light-sampling helpers ──────────────────────
// Power heuristic (Veach 1997, beta=2) — the standard MIS weight combining
// two unbiased single-sample estimators (light sampling vs BSDF sampling) of
// the SAME integral so neither technique's variance (a tiny bright emitter
// for BSDF sampling; a grazing/occluded sample for light sampling) dominates.
fn power_heuristic(pdf_a: f32, pdf_b: f32) -> f32 {
  let a2 = pdf_a * pdf_a;
  let b2 = pdf_b * pdf_b;
  let sum = a2 + b2;
  return select(a2 / sum, 0.0, sum <= 0.0);
}

// Uniform-by-area point on triangle (v0,v1,v2) via the sqrt-barycentric warp
// (Shirley & Chiu) — the standard unit-square-to-unit-triangle map whose
// Jacobian cancels the sqrt(u1) term, so the result is exactly area-uniform.
fn sample_triangle(v0: vec3<f32>, v1: vec3<f32>, v2: vec3<f32>, u1: f32, u2: f32) -> vec3<f32> {
  let su = sqrt(u1);
  let b0 = 1.0 - su;
  let b1 = u2 * su;
  return v0 * b0 + v1 * b1 + v2 * (1.0 - b0 - b1);
}

// Binary-search the emissive-light CDF (POWER-weighted — see `Light` /
// `build_emissive_lights` host-side) for `u1` in [0,1) → light index.
// O(log N); callers gate on `arrayLength(&lights) > 0u` first, so `lights`
// is never empty here.
fn sample_light_index(u1: f32) -> u32 {
  ...
  loop {
    if (lo >= hi) { break; }
    let mid = (lo + hi) / 2u;
    if (lights[mid].cdf < u1) { lo = mid + 1u; } else { hi = mid; }
  }
  return lo;
}

// MAGIC CRYSTAL PART 2: firefly clamp for the emissive-NEE shadow-ray term.
// `1/pdf_light_sa` is a near-emitter singularity (pdf_light_sa -> 0 as a
// shading point approaches a light's plane at grazing `cos_light`, or as
// `dist2` shrinks) — an UNBOUNDED per-sample spike a finite-spp render can
// never average away (a classic firefly). `clamp_v <= 0.0` is the IRON
// default (GAIA_NEE_CLAMP unset/0 => off, byte-identical to the unclamped
// term). When on, scale the whole RGB contribution down by the SAME factor
// (max-channel radiance clamp) so hue/chromaticity is preserved and only
// magnitude is capped — the standard unbiased-in-expectation-only /
// consistent firefly suppression used by every offline path tracer's
// "clamp" dial (biased per-sample, but the bias shrinks as spp grows since
// only the rare outlier samples are touched).
fn clamp_nee_radiance(c: vec3<f32>, clamp_v: f32) -> vec3<f32> {
  if (clamp_v <= 0.0) { return c; }
  let m = max(c.x, max(c.y, c.z));
  if (m <= clamp_v) { return c; }
  return c * (clamp_v / m);
}
...
// ── MAGIC CRYSTAL STAGE 1/2: emissive-triangle NEE + parameterized mirror
// threshold ───────────────────────────────────────────────────────────────
// `radiance_nee` is `radiance`'s STAGE 1/2 twin: emissive-triangle NEE
// (light sampling + shadow ray + power-heuristic MIS against BSDF sampling,
// diffuse lobe only). CORRECTED 2026-07-31 (adversary FAILS,
// docs/adversary/2026-07-28-v9body-merge.md Finding 4): the light-sampling
// term now carries the Lambert f_r = (1-metallic)*albedo/π, matching the
// BSDF-sampling technique's implicit f_r exactly (that bounce multiplies
// throughput by plain `albedo` and its pdf_bsdf_sa already carries the
// missing 1/π at `prev_bsdf_pdf_sa`/`pdf_bsdf_sa` below) — the two MIS
// partners now estimate the SAME integral, which is the MIS correctness
// condition. The PREVIOUS comment here claimed "no 1/π … so nothing
// double-counts" — that claim was false; see the adversary report for the
// Monte-Carlo proof (~3.14x bias). The sun's shadow ray a few lines above
// legitimately omits the 1/π because it is a delta light with a free
// intensity dial (`sun_color.w`) that absorbs it — that exemption does NOT
// transfer to `tri.emission`, which is the same radiance the hit-emission
// technique adds with no such free dial. Also carries the parameterized
// mirror threshold (`u.misc.w`, IRON default 1e-3 = the old hardcoded
// `MIRROR_ROUGHNESS`). A SEPARATE function (not `radiance` in place)
// because it reads the STAGE 1 `lights` global (@group(2)) —
// `radiance` stays byte-unchanged so `integrate_temporal` (already at the
// 8-storage-buffer/stage cap with its own @group(1)) never needs that
// binding. Used by `integrate` only; `radiance_ed`/`integrate_split` carries
// the matching E/D-bucketed twin of this same construction.
fn radiance_nee(ray_o: vec3<f32>, ray_d: vec3<f32>, pixel: u32, sample: u32, first_hit: Hit) -> vec3<f32> {
  var o = ray_o;
  var d = ray_d;
  var throughput = vec3<f32>(1.0, 1.0, 1.0);
  var L = vec3<f32>(0.0, 0.0, 0.0);
  ...
  let rr_start = u32(u.misc.z);
  let max_bounces = u.params.w;
  let mirror_max = u.misc.w;
  let nee_on = u.surface.w > 0u;
  let nee_clamp = bitcast<f32>(u.temporal_flags.w);
  var bounce = 0u;
  var pending_hit = first_hit;
  var have_pending = true;
  // MIS bookkeeping for the PREVIOUS bounce's diffuse-lobe BSDF sample (see
  // the hit-emission block below): whether it was taken, and the
  // solid-angle density it assigned to the direction actually followed.
  var prev_diffuse = false;
  var prev_bsdf_pdf_sa = 0.0;
  loop {
    var hit: Hit;
    if (have_pending) {
      hit = pending_hit;
    ...
    // Emissive surfaces glow at every hit — MIS-weighted against the
    // emissive-NEE shadow ray taken at the PREVIOUS diffuse-lobe hit (if
    // any), so a bounce that lands back on the same light this shadow ray
    // could have reached is not double-counted. Any of: NEE off, this is the
    // primary ray (bounce 0 — NEE never samples the camera), the previous
    // bounce wasn't the diffuse lobe, or this triangle isn't a registered
    // light → full weight 1 (byte-identical to plain hit-emission).
    var mis_w = 1.0;
    if (nee_on && bounce > 0u && prev_diffuse) {
      let tag = i32(round(tri.v0.w));
      if (tag > 0) {
        let light = lights[u32(tag - 1)];
        let cos_light = abs(dot(n, d));
        if (cos_light > 1e-6 && light.area > 0.0) {
          let dist2 = hit.t * hit.t;
          let pdf_light_sa = light.pdf / light.area * dist2 / cos_light;
          if (pdf_light_sa > 0.0) {
            mis_w = power_heuristic(prev_bsdf_pdf_sa, pdf_light_sa);
          }
        }
      }
    }
    L = L + throughput * tri.emission.xyz * mis_w;
    ...
    // Next-event: an emissive-triangle light via a shadow ray. Diffuse lobe
    // only (same restriction as the sun above). Power-heuristic MIS against
    // the diffuse-lobe BSDF-sampling technique (which would estimate the same
    // direct-lighting integral by bouncing here and landing on the light via
    // hit-emission above) — see `sample_light_index`/`Light` for the
    // power-weighted light pick and the host-side build.
    if (nee_on && metallic < 1.0) {
      let n_lights = arrayLength(&lights);
      if (n_lights > 0u) {
        let base_nee = max_bounces * 4u + bounce * 3u;
        let ul = urand(pixel, sample, base_nee + 0u);
        let u1 = urand(pixel, sample, base_nee + 1u);
        let u2 = urand(pixel, sample, base_nee + 2u);
        let li = sample_light_index(ul);
        let light = lights[li];
        if (light.tri != hit.tri) {
          let ltri = tris[light.tri];
          let lp = sample_triangle(ltri.v0.xyz, ltri.v1.xyz, ltri.v2.xyz, u1, u2);
          let to_light = lp - p;
          let dist2 = dot(to_light, to_light);
          let dist = sqrt(max(dist2, 1e-12));
          let wi = to_light / dist;
          let ndl2 = dot(n, wi);
          if (ndl2 > 0.0) {
            let lnormal = normalize(cross(ltri.v1.xyz - ltri.v0.xyz, ltri.v2.xyz - ltri.v0.xyz));
            let cos_light = abs(dot(lnormal, wi));
            if (cos_light > 1e-6 && light.area > 0.0) {
              let pdf_light_sa = (light.pdf / light.area) * dist2 / cos_light;
              if (pdf_light_sa > 0.0) {
                let pdf_bsdf_sa = (1.0 - metallic) * ndl2 * (1.0 / PI);
                let mis = power_heuristic(pdf_light_sa, pdf_bsdf_sa);
                if (!occluded(p + n * eps, wi, eps, dist - eps)) {
                  let light_c = clamp_nee_radiance(throughput * (1.0 - metallic) * tri.albedo.xyz
                        * (1.0 / PI) * ltri.emission.xyz * ndl2 / pdf_light_sa * mis, nee_clamp);
                  L = L + light_c;
                }
              }
            }
    ...
    if (u_lobe < metallic) {
      ...
      throughput = throughput * tri.albedo.xyz;
      prev_diffuse = nee_on;
      prev_bsdf_pdf_sa = (1.0 - metallic) * max(dot(n, dir), 0.0) / PI;
    }
    if (max(throughput.x, max(throughput.y, throughput.z)) <= 0.0) { break; }
...
fn radiance_ed(ray_o: vec3<f32>, ray_d: vec3<f32>, pixel: u32, sample: u32, first_hit: Hit) -> RadPair {
  ...
  var pending_hit = first_hit;
  var have_pending = true;
  // STAGE 1 MIS bookkeeping — see `radiance_nee` (byte-identical
  // construction); the only difference here is bucketing every new
  // contribution into e_acc/d_acc like every other term this function
  // already computes.
  var prev_diffuse = false;
  var prev_bsdf_pdf_sa = 0.0;
  loop {
    ...
    // STAGE 1: MIS-weight the hit-emission against the emissive-NEE shadow
    // ray taken at the PREVIOUS diffuse-lobe hit (if any) — see
    // `radiance_nee` for the full rationale; identical construction here.
    var mis_w = 1.0;
    if (nee_on && bounce > 0u && prev_diffuse) {
      let tag = i32(round(tri.v0.w));
      if (tag > 0) {
    ...
    let emis = throughput * tri.emission.xyz * mis_w;
    ...
    // STAGE 1: emissive-triangle NEE shadow ray — see `radiance_nee` for the
    // full construction; identical here, just bucketed like the sun term.
    if (nee_on && metallic < 1.0) {
      let n_lights = arrayLength(&lights);
      if (n_lights > 0u) {
        let base_nee = max_bounces * 4u + bounce * 3u;
        let ul = urand(pixel, sample, base_nee + 0u);
        let u1n = urand(pixel, sample, base_nee + 1u);
        let u2n = urand(pixel, sample, base_nee + 2u);
        let li = sample_light_index(ul);
        let light = lights[li];
        if (light.tri != hit.tri) {
          let ltri = tris[light.tri];
          let lp = sample_triangle(ltri.v0.xyz, ltri.v1.xyz, ltri.v2.xyz, u1n, u2n);
          let to_light = lp - p;
          let dist2 = dot(to_light, to_light);
          let dist = sqrt(max(dist2, 1e-12));
          let wi = to_light / dist;
            ...
            if (cos_light > 1e-6 && light.area > 0.0) {
              ...
              if (pdf_light_sa > 0.0) {
                ...
                if (!occluded(p + n * eps, wi, eps, dist - eps)) {
                  let light_c = clamp_nee_radiance(throughput * (1.0 - metallic) * tri.albedo.xyz
                        * (1.0 / PI) * ltri.emission.xyz * ndl2 / pdf_light_sa * mis, nee_clamp);
                  if (diffuse_seen) { d_acc = d_acc + light_c; } else { e_acc = e_acc + light_c; }
                }
              }
            }
          }
...
fn integrate(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var s = 0u; s < spp; s = s + 1u) {
    ...
    // STAGE 1/2: `radiance_nee` (emissive NEE + parameterized mirror
    // threshold) — `radiance` proper stays untouched for `integrate_temporal`.
    let surf = radiance_nee(u.eye.xyz, dir, pixel, sample, prim);
