//! R-DIRECT v9i-body — CURE 5, ACTIVATE THE MUTE PIXEL CLASS
//! (`scratch/v9-autopsy.md` v9h grad-probe-fix section, this round). Base:
...
fn main() {
    let run_tag = std::env::var("GAIA_V9_TAG").unwrap_or_else(|_| "v9i".to_string());
    ...
    'train: for epoch in 0..epochs {
        ...
        for &(pi, s) in &order {
            ...
            for px in 0..(out.h * out.w) {
                ...
                let is_nohit = albedo[px].length_squared() <= nohit_albedo_sq;
                for c in 0..out.c {
                    let i = px * out.c + c;
