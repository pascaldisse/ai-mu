//! R-DIRECT v9c-body — DOMAIN FIX + POSE DIVERSITY ROUND (v9c task,
//! `scratch/v9-autopsy.md`). Base: `rdirect_train_v9b.rs` (the CURED round —
//! CURE 1 evidence-clamped loss + CURE 2 abort-on-detonation watchdog,
//! unchanged, see below) plus TWO new fixes:
//!
//! DOMAIN FIX (autopsy conviction #2, also found live in `settle()`/
...
fn main() {
    let run_tag = std::env::var("GAIA_V9_TAG").unwrap_or_else(|_| "v9c".to_string());
    let abort_streak = env_u32("GAIA_V9C_ABORT_STREAK", 20); // CURE 2
    let pool_orbit_deg = env_f32("GAIA_V9C_POOL_ORBIT_DEG", 30.0); // POSE DIVERSITY
    let pool_jitter = env_f32("GAIA_V9C_POOL_JITTER", 0.6); // POSE DIVERSITY, world units
    ...
    eprintln!(
        "[{run_tag}] POSE DIVERSITY POOL: orbit±{pool_orbit_deg}deg jitter±{pool_jitter} around 2 anchors (front-pivot family x2 draws/epoch, wide-anchor family x1 draw/epoch) + mirror unconditional (fixed) — fresh draws every epoch, see module doc"
    );
    ...
    'train: for epoch in 0..epochs {
        ...
        let poses: Vec<PoseSeq> = vec![
            render_pose_seq(&device, &queue, &base_tris, &scene, &draw_a1, k, low_w, low_h, tw, th, ref_frames, pan_step, 1, k_draws),
            render_pose_seq(&device, &queue, &base_tris, &scene, &draw_a2, k, low_w, low_h, tw, th, ref_frames, pan_step, 1, k_draws),
            render_pose_seq(&device, &queue, &base_tris, &scene, &draw_b, k, low_w, low_h, tw, th, ref_frames, pan_step, 1, k_draws),
            mirror_seq.clone(),
        ];
    ...
    let prov = serde_json::json!({
        ...
        "training": {
            ...
            "pose_diversity": { "pool_orbit_deg": pool_orbit_deg, "pool_jitter": pool_jitter, "draws_per_epoch": 3, "anchors": ["front-pivot (x2 draws/epoch)", "wide-anchor (x1 draw/epoch)"], "note": "orbit yaw in [-pool_orbit_deg,+pool_orbit_deg] about each anchor's pivot + small eye-position jitter box (±pool_jitter world units, vertical ×0.4), fresh draw every epoch — replaces v9b's 3 literal-fixed cameras (front/wide/orbit_+20)" },
        ...
        "gate": "NOT ordealed — v9c output only, no bar claimed passed",
    });
