//! V9T PIXEL-TRUTH TRIPTYCH — writes THREE fixed-name 640x480 PNGs from
//! the SAME bar-res pose so ground truth is directly eyeballable:
//!   scratch/v9t-teacher.png = converged K-draw-averaged tracer target
//!   scratch/v9t-net.png     = v9t net's own composed bar-res output
//!   scratch/v9t-diff.png    = per-px |net-teacher| luminance heatmap
//!                             (black=0, white>=0.15, same SPARK_DELTA)
//! Forked from `examples/rdirect_v9t_visual.rs` — SAME bar-res compose
//! pipeline (K=3 recurrent settle + v9n hit-gate + v9p dark-cap +
//! normal-ceiling branches, byte-for-byte from
//! `rdirect_train_v9k.rs::bar_res_probe`), sparkle-marked outputs dropped
//! in favor of the diff heatmap.
//!
...
fn main() {
    ...
    eprintln!("[v9t-triptych:{tag}] {} sparkle-flagged texels ({sp:.1}/Mpx) at {tw}x{th}", flagged.len());
    println!("[v9t-triptych:{tag}] weights={wpath:?} flagged={} sparkle={sp:.1}/Mpx", flagged.len());

    let out_dir = env_str("GAIA_V9_EVAL_OUT", "scratch");

    // (a) teacher — converged K-draw-averaged tracer target, plain sRGB.
    let teacher_bytes = img_to_srgb_bytes(&teacher, 1.0);
    write_png_rgb_bytes(&teacher_bytes, tw, th, Path::new(&out_dir).join("v9t-teacher.png").as_path());

    // (b) net — v9t's own composed bar-res output, plain sRGB.
    let net_bytes = img_to_srgb_bytes(&net_clamped, 1.0);
    write_png_rgb_bytes(&net_bytes, tw, th, Path::new(&out_dir).join("v9t-net.png").as_path());
    ...
    // sparkle criterion itself uses (`lum`/SPARK_DELTA above).
    ...
    write_png_rgb_bytes(&diff_bytes, tw, th, Path::new(&out_dir).join("v9t-diff.png").as_path());
