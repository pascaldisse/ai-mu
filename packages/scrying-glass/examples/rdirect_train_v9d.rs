//! R-DIRECT v9d-body — WIDER POOL + EMA-EVAL ROUND (v9d task,
//! `scratch/v9-autopsy.md` v9c section). Base: `rdirect_train_v9c.rs`
//! (DOMAIN FIX + POSE DIVERSITY + CURE 1 evidence-clamp + CURE 2
//! abort-on-detonation watchdog, unchanged) plus TWO new deltas, per the
//! v9c autopsy verdict ("pose diversity delays and deepens but does not
//! cure memorization-onset"):
//!
//! CURE (a) — POOL MUCH WIDER. v9c drew 3 poses/epoch from 2 anchors at
//! ±30° yaw. v9d draws from 4 anchors (`front`/`wide`, kept, PLUS `high`
//! — same front pivot, eye raised to y=12 — and `close` — same front
//! pivot, eye pulled in to z=10 — the "varied heights/radii" the task
//! asks for, all still real naruko geometry, no new scene content) at a
//! FULL-ORBIT yaw range (`GAIA_V9D_POOL_ORBIT_DEG` default 180.0, was
//! v9c's 30.0 — `pool_camera`'s `orbit_camera` call is unchanged math, only
//! the sampled range widens) and `GAIA_V9D_POOL_DRAWS_PER_ANCHOR` (default
//! 2) draws per anchor per epoch — 8 pool draws/epoch (was 3) + the fixed
//! mirror pose = 9 poses/epoch (was 4). Jitter also widens
//! (`GAIA_V9D_POOL_JITTER` default 0.9, was v9c's 0.6). All four numbers
//! are env-tunable with the stated defaults (IRON: no bare literals without
//! an override knob) — see `main` for the anchor table and draw loop.
//!
//! CURE (b) — EMA WEIGHTS EVAL, RAW LOGGED ALONGSIDE. v9c already
//! maintained Polyak EMA weights (`ema.ema_update(&net, ema_decay)`,
//! `GAIA_V9_EMA` default 0.999) and already evaluated/checkpointed the EMA
//! net exclusively (`run_monitor(..., &ema, ...)`, `best_bytes =
//! serialize_weights(ema)`) — that machinery is UNCHANGED here, monitor/
//! checkpoint/best-floor keep evaluating EMA only, per the task. What v9d
//! ADDS: `run_monitor` now also `settle()`s the RAW (non-EMA) `net` against
//! the same held-out val/mirror poses and prints its sparkle/resid/score
//! alongside the EMA numbers on the SAME monitor line — diagnostic only,
//! never touches `best_score`/`rising_streak`/the abort watchdog/the
//! checkpoint file (those stay EMA-only, unchanged from v9c) — lets a
//! human compare "EMA curve smoother than raw?" per-epoch from one log
//! without a second run.
//!
//! Everything else below (CURE 1, CURE 2, DOMAIN FIX, the whole v7/v8/v9c
//! mechanism lineage) is v9c's own doc, kept verbatim for provenance —
//! read it as "inherited unchanged" wherever it doesn't mention v9d above.
//!
//! Run: cargo run --release -j2 --example rdirect_train_v9d
//!   GAIA_V7_SKY_HISTORY=reject GAIA_V9_K=8 (mandate defaults)
//!
//! ── inherited v9c doc (unchanged mechanisms) ──
//!
//! `run_monitor` here and in `rdirect_train_v9.rs`/`rdirect_train_v9b.rs`,
//! same bug, now fixed in all three): the net's raw output is demod-log
//! radiance (`rdirect_unet.rs` module doc), but `settle()` used to return it
//! AS-IS and compare directly against the LINEAR teacher. Fixed: the
//! recurrent chain still feeds RAW demod-log history (unchanged contract),
//! but the FINAL step's output is `undo_log_demod`'d (via that pixel's
//! albedo divisor, duplicated math from `rdirect.rs` — private there, same
//! approach as `rdirect_v9_eval_640.rs`) before every metric — so
//! `run_monitor`'s printed sparkle/resid/score are now on the SAME footing
//! as `rdirect_v9_eval_640.rs`'s and v8d's own numbers.
//!
//! POSE DIVERSITY (monad diagnosis: 4 LITERAL-FIXED training poses the
//! whole run = a conv overfit engine — a whole-image conv forward sees
//! `n_poses` samples/epoch where v8d's per-pixel net saw millions of
//! sampled pixels; v9's deterministic detonation onset at epoch 12 tracks
//! memorization onset, not the loss-clamp, already exonerated by v9b's own
//! clamp cure alone not preventing further runaway in ablation reasoning).
//! Training poses are now drawn FRESH every epoch from a parameterized pool
//! (`pool_camera`, below — orbit yaw + small position jitter around the
//! SAME two anchors `denoiser_dataset::law_poses` already authors, no new
//! scene geometry). Validation poses (`orbit_-20` still pose, `mirror`
//! still pose) stay the trainer's FIXED, literally held-out cameras —
//! UNCHANGED from v9/v9b, so scores remain a valid cross-run floor
//! comparison. See `pool_camera`'s doc comment for the full design.
//!
//! Everything else below is v9b's CURED recipe, unchanged:
//!
...
//! K-step temporal mean once PER EPOCH (a pose's ceiling is recomputed
//! every epoch now, since the pose itself is freshly drawn every epoch —
//! same per-pose-temporal-mean contract as v9b, just re-derived per draw)
//! and reuses that ceiling for every step of that pose within the epoch.
//!
...
//! epoch within 5% of best) resets it to 0. `GAIA_V9C_ABORT_STREAK`
...
/// CURE (b), v9d: raw (non-EMA) `net`'s val score, computed/logged
/// ALONGSIDE the EMA monitor line — diagnostic only, never feeds
/// `best_score`/`rising_streak`/the checkpoint (those stay EMA-only, see
/// module doc). Returns (sparkle, resid, score) for the raw net, same
/// metric shape as the EMA path below.
fn raw_monitor_probe(net: &UnetWeights, val_seq: &PoseSeq, sky_reject: bool) -> (f64, f64, f64) {
    let (n, t) = settle(net, val_seq, sky_reject);
...
fn run_monitor(
    ...
    let score = (sp / 40.0).max(rs / 0.035);
    // CURE (b): raw-weights probe, diagnostic-only (module doc) — computed
    // AFTER the EMA-driving decisions above so it can never influence them.
    let (raw_sp, raw_rs, raw_score) = raw_monitor_probe(raw_net, val_seq, sky_reject);
    ...
    eprintln!(
        "[{run_tag}] MONITOR {tag}: val sparkle {sp:.1}/Mpx resid {rs:.4} highlight_ratio {hl:.3} score={score:.3}{} | mirror sparkle {msp:.1}/Mpx resid {mrs:.4} highlight_ratio {mhl:.3} (tgt sp<{spark_target} resid<{resid_gate}){} rising_streak={rising_streak}/{abort_streak} | raw(non-EMA) sparkle {raw_sp:.1}/Mpx resid {raw_rs:.4} score={raw_score:.3}",
...
fn main() {
    let run_tag = std::env::var("GAIA_V9_TAG").unwrap_or_else(|_| "v9d".to_string());
    let abort_streak = env_u32("GAIA_V9C_ABORT_STREAK", 20); // CURE 2, kept
    let pool_orbit_deg = env_f32("GAIA_V9D_POOL_ORBIT_DEG", 180.0); // CURE (a): full-orbit, was v9c's 30.0
    let pool_jitter = env_f32("GAIA_V9D_POOL_JITTER", 0.9); // CURE (a): wider, was v9c's 0.6
    let pool_draws_per_anchor = env_u32("GAIA_V9D_POOL_DRAWS_PER_ANCHOR", 2); // CURE (a): draws/epoch ↑
    ...
    // POOL ANCHORS (CURE (a), module doc) — v9c's 2 anchors (front/wide,
    // `denoiser_dataset::law_poses`' own constants, duplicated here — that
    // fn doesn't expose them separately) PLUS 2 new anchors sharing the
    // front pivot but at a VARIED height ("high": eye raised to y=12) and a
    // VARIED radius ("close": eye pulled in to z=10) — real naruko geometry
    // viewed from new distances/elevations, no new scene content.
    ...
    let high_eye = [0.0f32, 12.0, 22.0]; // CURE (a): front pivot, raised height
    let close_eye = [0.0f32, 2.0, 10.0]; // CURE (a): front pivot, closer radius
    let anchors: [(&str, [f32; 3], [f32; 3]); 4] = [
        ("front", front_eye, front_pivot),
        ("wide", wide_eye, wide_pivot),
        ("high", high_eye, front_pivot),
        ("close", close_eye, front_pivot),
    ];
    eprintln!(
        "[{run_tag}] POOL MUCH WIDER: orbit±{pool_orbit_deg}deg (full-orbit) jitter±{pool_jitter} around 4 anchors (front/wide/high/close) x {pool_draws_per_anchor} draws/epoch each = {} pool draws/epoch + mirror unconditional (fixed) — fresh draws every epoch, see module doc CURE (a)",
        anchors.len() as u32 * pool_draws_per_anchor
    );

    let t_render = Instant::now();
    let mirror_seq = render_pose_seq(&device, &queue, &base_tris, &scene, &mirror_cam, k, low_w, low_h, tw, th, ref_frames, pan_step, mirror_spp, k_draws);
    ...
    run_monitor(&run_tag, "epoch -1 (fresh He-init)", &ema, &net, &val_seq, &mirror_val_seq, sky_reject, highlight_pctl, spark_target, resid_gate, &mut best_score, &mut best_bytes, &wpath, &mut rising_streak, abort_streak);
    ...
    'train: for epoch in 0..epochs {
        ...
        // CURE (a), POOL MUCH WIDER (module doc): pool_draws_per_anchor fresh
        // draws PER anchor (4 anchors: front/wide/high/close) this epoch +
        // the FIXED mirror sequence cloned in — replaces v9c's 2-anchor,
        // 3-draw pool.
        let t_pool = Instant::now();
        let mut poses: Vec<PoseSeq> = Vec::with_capacity(anchors.len() * pool_draws_per_anchor as usize + 1);
        let mut pool_yaws: Vec<f32> = Vec::with_capacity(anchors.len() * pool_draws_per_anchor as usize);
        for &(_name, eye, pivot) in anchors.iter() {
            for _ in 0..pool_draws_per_anchor {
                let (draw, yaw) = pool_camera(eye, pivot, fov_deg, pool_orbit_deg, pool_jitter, &mut rng);
                pool_yaws.push(yaw);
                poses.push(render_pose_seq(&device, &queue, &base_tris, &scene, &draw, k, low_w, low_h, tw, th, ref_frames, pan_step, 1, k_draws));
            }
        }
        poses.push(mirror_seq.clone());
        ...
        println!(
            "[{run_tag}] epoch {}/{} n2n_mse={:.6} lr={:.6} pool_yaws={pool_yaws:.1?} pool_ms={pool_ms:.0} hist_ms={hist_ms:.0} batch_ms={batch_ms:.0} ({:.1}s)",
            epoch, epochs, mse_mean, adam.lr(), t_train.elapsed().as_secs_f64()
        );
        std::io::stdout().flush().ok();

        if (epoch + 1) % monitor_every == 0 || epoch + 1 == epochs {
            let (_, _, should_abort) = run_monitor(&run_tag, &format!("epoch {epoch}"), &ema, &net, &val_seq, &mirror_val_seq, sky_reject, highlight_pctl, spark_target, resid_gate, &mut best_score, &mut best_bytes, &wpath, &mut rising_streak, abort_streak);
            if should_abort {
    ...
    let prov = serde_json::json!({
        ...
        "training": {
            ...
            "pose_diversity": { "pool_orbit_deg": pool_orbit_deg, "pool_jitter": pool_jitter, "draws_per_epoch": anchors.len() as u32 * pool_draws_per_anchor, "draws_per_anchor": pool_draws_per_anchor, "anchors": ["front", "wide", "high (front pivot, raised eye y=12)", "close (front pivot, closer eye z=10)"], "note": "CURE (a) v9d: full-orbit yaw in [-pool_orbit_deg,+pool_orbit_deg] about each anchor's pivot + eye-position jitter box (±pool_jitter world units, vertical ×0.4), fresh draw every epoch, draws_per_anchor draws per anchor, 4 anchors (was v9c's 2 anchors, 3 draws total) — replaces v9c's 2-anchor/3-draw pool" },
            "domain_fix": "settle()/run_monitor now undo_log_demod the final step's output (per-pixel albedo divisor) before every metric — same fix as rdirect_v9_eval_640.rs, ported back into the trainer's own in-loop monitor (scratch/v9-autopsy.md conviction #2), unchanged from v9c",
            "ema": { "decay": ema_decay, "note": "CURE (b) v9d: Polyak EMA weights (kept from v9c/cf8bd7b) — monitor/checkpoint/best-floor/abort-watchdog evaluate ONLY the EMA net; the raw (non-EMA) net's val score is now ALSO settled + logged every monitor line (module doc CURE (b)) but never drives best_score/rising_streak/the checkpoint" },
        },
        "dataset": { "realm": "naruko", "low": [low_w, low_h], "native": [tw, th],
            "train": ["front-pool", "wide-pool", "high-pool", "close-pool", "mirror (fixed, unconditional)"], "val": ["orbit_-20 (still, FIXED, held-out)", "mirror (still, FIXED, held-out)"] },
        "gate": "NOT ordealed — v9d output only, no bar claimed passed",
    });
