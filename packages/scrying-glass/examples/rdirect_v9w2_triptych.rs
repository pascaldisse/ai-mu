//! V9W2 PIXEL-TRUTH TRIPTYCH — writes THREE fixed-name 640x480 PNGs from
//! the SAME bar-res pose so ground truth is directly eyeballable:
//!   scratch/v9w2-teacher.png = converged-teacher tracer target (default
//!                              512spp chunked accumulation, matches training)
//!   scratch/v9w2-net.png     = v9w2 net's own composed bar-res output
//!   scratch/v9w2-diff.png    = per-px |net-teacher| luminance heatmap
//!                              (black=0, white>=0.15, same SPARK_DELTA)
//!
//! Forked from `examples/rdirect_v9t_triptych.rs`, re-matched to v9w2's
//! ACTUAL training config (scratch/v9w2-train.log) — v9t's tool is NOT
//! byte-compatible eval for v9w2, three real differences:
//!
//!  1. INPUT CLAMP (V9V, `examples/rdirect_train_v9k.rs::apply_input_clamp`)
//!     — v9w2 trained with `GAIA_V9V_INPUT_CLAMP=0.10` (confirmed indirectly:
//!     the train log's `input_clamp_taps` counter is non-zero every epoch,
//!     e.g. epoch0=55866 — the counter is a hard 0 whenever the delta is
//!     <=0.0, so it cannot be non-zero unless the env was set). This clamp
//!     lives ONLY in `rdirect_train_v9k.rs` (ported verbatim below, NOT in
//!     the shared `src/rdirect.rs` feature builders) and is applied right
//!     after `pixel_features_split` at each of that file's 3 call sites —
//!     v9t's triptych never called it at all. Default here: 0.10.
//!  2. DARK-HIT COMPOSE (V9P/V9Q) — v9w2 trained with
//!     `GAIA_V9P_DARK_CAP_DELTA=0.10` AND `GAIA_V9Q_DESPECKLE_DELTA=0.10`
//!     (both are `rdirect_train_v9k.rs` FUNCTION DEFAULTS, unlike v9t which
//!     explicitly zeroed both — "no caps", scratch/v9t-train.log lines
//!     9-11). V9Q takes priority over V9P at a dark-hit px whenever both
//!     deltas are positive, so v9w2's REAL compose is: no-hit→evidence
//!     passthrough (V9N), dark-hit→net output kept THROUGH a POST-COMPOSE
//!     linear cap against the composed frame's own 3x3 neighborhood max
//!     + delta (V9Q `despeckle_cap_lin`, two-pass: precap_lin frozen, then
//!     capped), everything else→the ordinary evidence ceiling. Ported
//!     verbatim from `bar_res_probe`'s own two-pass block below.
//!  3. TEACHER (V9U) — v9w2 trained with `GAIA_V9_TEACHER_SPP=512
//!     chunk=100` (scratch/v9w2-train.log line 13: "teacher 512spp
//!     CONVERGED"), so the bar-res probe's OWN judge during training was a
//!     true converged render, not a 64-frame `ref_frames` average. Ported
//!     `render_converged_lin` (chunked accumulation, byte-for-byte
//!     `rdirect_train_v9k.rs`/`rdirect_reference.rs::render_converged`) as
//!     this tool's teacher when `GAIA_V9_TEACHER_SPP>0` (default HERE:
//!     512, matching v9w2's own training judge — NOT the v9t tool's
//!     `ref_frames=64` default, which would silently mismatch).
//!
//! Env: GAIA_V9W2_WEIGHTS (default data/rdirect-weights-v9w2-last.bin),
//! GAIA_V9W2_TAG (default "eval" — output filename suffix),
//! GAIA_V9N_HITGATE / GAIA_V9O_DARK_ALB / GAIA_V9P_DARK_CAP_DELTA /
//! GAIA_V9Q_DESPECKLE_DELTA / GAIA_V9V_INPUT_CLAMP (override for A/B,
//! matching v9k/v9p/v9t tool conventions), GAIA_V9_TEACHER_SPP /
//! GAIA_V9_TEACHER_CHUNK (converged-teacher accumulation),
//! GAIA_V7_SKY_HISTORY=reject (matches training; NOT defaulted on here —
//! set it explicitly to replicate the v9w2 run).
//!
//! GAIA_V9V_MOTION is intentionally NOT reproduced here: it only feeds
//! `render_pose_seq_cropped`'s POOL-DRAW camera trajectory in the training
//! file (`motion_on` is referenced there only), never `bar_res_probe`'s
//! feature loop — the probe (and this tool) uses the SAME fixed `val_cam`
//! pose at every K step, so its own reprojected motion vector is ~0
//! regardless of the training-time motion flag: static pose => mv=0 is the
//! trained behavior, not a mismatch (task mandate, confirmed by code read).
//!
//! Run: GAIA_V7_SKY_HISTORY=reject GAIA_V9W2_WEIGHTS=data/rdirect-weights-v9w2-last.bin \
//!      GAIA_V9V_INPUT_CLAMP=0.10 cargo run --release -j2 --example rdirect_v9w2_triptych
use std::io::Write;
use std::path::Path;
...
/// COPIED from `rdirect_v9p_barres_forensics.rs::hitgate_on` — v9w2 trained
...
/// v9w2 trained with `GAIA_V9O_DARK_ALB=0.0001` (the function's own default
/// in `rdirect_train_v9k.rs`, unchanged here).
fn v9o_dark_alb() -> f32 {
    env_f32("GAIA_V9O_DARK_ALB", 1.0e-4)
}
/// v9w2 trained with `GAIA_V9P_DARK_CAP_DELTA=0.10` (default in
/// `rdirect_train_v9k.rs`) — UNLIKE v9t which zeroed this ("no caps").
/// V9Q (below) takes priority whenever ITS delta is also positive, which
/// it is for v9w2, so this branch is a fallback that should rarely engage.
fn v9p_dark_cap_delta() -> f32 {
    env_f32("GAIA_V9P_DARK_CAP_DELTA", 0.10)
}
/// V9Q AIRTIGHT DESPECKLE CAP (module doc / `rdirect_train_v9k.rs`) — v9w2
/// trained with `GAIA_V9Q_DESPECKLE_DELTA=0.10` (function default,
/// unchanged here). At dark-hit px, TAKES PRIORITY over V9P: net output is
/// kept uncapped through the PRE-cap compose, then POST-COMPOSE bounded to
/// at most its 3x3 neighbors' (excluding self, the frozen pre-cap frame)
/// max LINEAR radiance + delta. `delta<=0.0` disables (falls to V9P).
...
/// `rdirect_train_v9k.rs::v9v_input_clamp_delta` — v9w2 trained with
/// `GAIA_V9V_INPUT_CLAMP=0.10` (confirmed by non-zero `input_clamp_taps`
/// every epoch in scratch/v9w2-train.log). Default HERE: 0.10 (matches
/// v9w2's OWN training value — NOT the shared function's byte-identical-
/// disable default of 0.0, which would silently mismatch this eval).
...
/// V9U CONVERGED-TEACHER SPP — v9w2 trained with `GAIA_V9_TEACHER_SPP=512
/// chunk=100`. Default HERE: 512/100 (matches v9w2's OWN training judge —
...
fn write_png_rgb_bytes(bytes: &[u8], w: u32, h: u32, path: &Path) {
    ...
    eprintln!("[v9w2-triptych] wrote {}", path.display());
}
...
fn main() {
    let tag = env_str("GAIA_V9W2_TAG", "eval");
    ...
    eprintln!(
        "[v9w2-triptych:{tag}] GAIA_V7_SKY_HISTORY reject={sky_reject} GAIA_V9N_HITGATE gate_on={gate_on} GAIA_V9O_DARK_ALB={dark_alb} GAIA_V9P_DARK_CAP_DELTA={dark_cap_delta} GAIA_V9Q_DESPECKLE_DELTA={despeckle_delta} GAIA_V9V_INPUT_CLAMP={input_clamp} GAIA_V9_TEACHER_SPP={teacher_spp} chunk={teacher_chunk}"
    );

    let Some((device, queue)) = headless_device() else {
        panic!("[v9w2-triptych:{tag}] no GPU");
    };
    ...
    let wpath = std::env::var("GAIA_V9W2_WEIGHTS")
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|_| data_dir.join("rdirect-weights-v9w2-last.bin"));
    ...
    eprintln!("[v9w2-triptych:{tag}] loaded {wpath:?} ({} bytes) sha256={}", bytes.len(), weights_sha256(&ema));
    eprintln!("[v9w2-triptych:{tag}] bar-res {tw}x{th} K={k} steps, ref_frames={ref_frames}");
    ...
    // v9w2's OWN training judge), else the old ref_frames average.
    ...
    eprintln!("[v9w2-triptych:{tag}] {} sparkle-flagged texels ({sp:.1}/Mpx) resid={resid:.4} at {tw}x{th}", flagged.len());
    println!("[v9w2-triptych:{tag}] weights={wpath:?} flagged={} sparkle={sp:.1}/Mpx resid={resid:.4}", flagged.len());

    let out_dir = env_str("GAIA_V9_EVAL_OUT", "scratch");

    // (a) teacher — converged tracer target, plain sRGB.
    let teacher_bytes = img_to_srgb_bytes(&teacher, 1.0);
    write_png_rgb_bytes(&teacher_bytes, tw, th, Path::new(&out_dir).join("v9w2-teacher.png").as_path());

    // (b) net — v9w2's own composed bar-res output, plain sRGB.
    let net_bytes = img_to_srgb_bytes(&net_clamped, 1.0);
    write_png_rgb_bytes(&net_bytes, tw, th, Path::new(&out_dir).join("v9w2-net.png").as_path());
    ...
    write_png_rgb_bytes(&diff_bytes, tw, th, Path::new(&out_dir).join("v9w2-diff.png").as_path());
