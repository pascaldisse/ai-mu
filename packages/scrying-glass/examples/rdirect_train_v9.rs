//! R-DIRECT v9-body STAGE 2 — trainer for the shape-parametric conv U-Net
//! body (`rdirect_unet.rs::cpu::UnetWeights`), extending the v7/v8 lineage's
//! OWN training recipe (this file is NOT a new trainer design — it is the
//! v8d recipe re-hosted on a whole-image conv net instead of a per-pixel
//! MLP; every mechanism below has a v7/v8 file it is copied/adapted from,
//! cited inline).
//!
...
use scrying_glass::rdirect::{
    CamPose, HIST_FEATURES_SPLIT, INPUT_FEATURES_SPLIT, OUTPUT_CHANNELS, hist_features_split,
...
/// One step of a moving-camera pose sequence — v8d's `Step`, minus the
/// evidence-clamp ceiling (not wired in `cpu::UnetWeights`, disclosed gap).
struct Step {
    low_e: Vec<GVec3>,
    low_d: Vec<GVec3>,
    albedo: Vec<GVec3>,
...
fn render_pose_seq(
    ...
    for step in 0..k {
        ...
        steps.push(Step { low_e, low_d, albedo, normal, depth, teacher, target_dl });
    }
...
/// `settle`/`direct_render_sequence_hist_split`). The recurrent chain is fed
/// RAW demod-log output at every step (unchanged — that is the net's own
/// history contract), but the RETURNED net image is undo_log_demod'd via the
/// FINAL step's per-pixel albedo divisor before comparison to the (already
/// linear) teacher — DOMAIN FIX, see const block above.
...
fn main() {
    let run_tag = std::env::var("GAIA_V9_TAG").unwrap_or_else(|_| "v9body".to_string());
    ...
    run_monitor(&run_tag, "epoch -1 (fresh He-init)", &ema, &val_seq, &mirror_val_seq, sky_reject, highlight_pctl, spark_target, resid_gate, &mut best_score, &mut best_bytes, &wpath);
    ...
    'train: for epoch in 0..epochs {
        ...
        for &(pi, s) in &order {
            ...
            // Plain MSE, whole-image, vs the K-averaged noise2noise label
            // (mechanisms 3+4) — teacher NEVER appears here.
            ...
            for i in 0..out.data.len() {
                ...
                d_out.data[i] = 2.0 * diff / n_elems; // d(mean-sq-err)/d(out)
            }
            epoch_mse += mse;
        ...
        if (epoch + 1) % monitor_every == 0 || epoch + 1 == epochs {
            run_monitor(&run_tag, &format!("epoch {epoch}"), &ema, &val_seq, &mirror_val_seq, sky_reject, highlight_pctl, spark_target, resid_gate, &mut best_score, &mut best_bytes, &wpath);
        }
    }
