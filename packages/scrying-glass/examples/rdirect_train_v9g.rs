//! R-DIRECT v9g-body — CURE 4: ASYMMETRIC-GRADIENT FIX (mechanism hunt,
//! `scratch/v9-autopsy.md` v9g section). Base: `rdirect_train_v9f.rs`
...
//! header) — the ONLY change is CURE 1's gradient formula at the loss call
//! site below.
//!
...
//! restoring force. `GAIA_V9G_GRAD_PROBE=1` (added this round, reused from
...
//!
//! CURE 4 (this file, IRON param `GAIA_V9G_OVERSHOOT_W`, default 1.0):
...
fn main() {
    let run_tag = std::env::var("GAIA_V9_TAG").unwrap_or_else(|_| "v9g".to_string());
    ...
    let overshoot_w = env_f32("GAIA_V9G_OVERSHOOT_W", 1.0); // CURE 4 (module doc): weight of the NEW above-ceiling restoring gradient
    ...
    // V9G MECHANISM-HUNT PROBE (c): opt-in, GAIA_V9G_GRAD_PROBE=1. Loads a
    // checkpoint (GAIA_V9G_PROBE_WEIGHTS, default the v9f-last artifact),
    ...
    if std::env::var("GAIA_V9G_GRAD_PROBE").ok().as_deref() == Some("1") {
        let data_dir0 = Path::new(env!("CARGO_MANIFEST_DIR")).join("data");
        let wpath = std::env::var("GAIA_V9G_PROBE_WEIGHTS")
        ...
        eprintln!("[v9g-grad-probe] loaded {wpath:?} ({} bytes)", bytes.len());
        ...
        eprintln!("[v9g-grad-probe] val_seq {}x{}, last step {last_s}, texel raw/cap/target/active/d_out per channel:", val_seq.tw, val_seq.th);
        for &(x, y) in texels {
            ...
            for c in 0..OUTPUT_CHANNELS {
                ...
                eprintln!(
                    "[v9g-grad-probe]   px=({x},{y}) c={c} raw={raw:.6} cap={cap:.6} target={target:.6} overshoot={} active={active} d_out={d_out:.8}",
                    raw > cap,
                );
            }
        }
        eprintln!("[v9g-grad-probe] done — exiting (no training run)");
        std::io::stderr().flush().ok();
        return;
    }
