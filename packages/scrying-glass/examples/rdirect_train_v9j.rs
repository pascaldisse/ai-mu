...
fn main() {
    let run_tag = std::env::var("GAIA_V9_TAG").unwrap_or_else(|_| "v9j".to_string());
    ...
    'train: for epoch in 0..epochs {
        ...
        for &(pi, s) in &order {
            ...
            let albedo = &seq.steps[s].albedo;
            let depth = &seq.steps[s].depth;
            let n_elems = out.data.len() as f32;
            ...
            for px in 0..(out.h * out.w) {
                ...
                for c in 0..out.c {
                    ...
                    let active = raw <= cap;
                    d_out.data[i] = if is_nohit {
                        // CURE 5: bypass clamp/ceiling — ordinary two-sided
                        // MSE against the honest target, always active.
                        2.0 * (raw - target.data[i]) / n_elems
                    } else if active {
                        2.0 * diff / n_elems
                    } else {
            ...
            if first_batch {
                first_batch = false;
                eprintln!(
                    "[{run_tag}] FIRST BATCH mask check (epoch {epoch}, pi={pi} s={s}): nohit_px(albedo)={batch_nohit_px}/{} nohit_effective(differs-from-CURE4)={batch_nohit_effective} | cross-check vs depth-hit-flag: false_hit_dark(albedo-mask says nohit, depth says REAL HIT)={batch_false_hit_dark} false_miss_albedo(depth says miss, albedo-mask missed it — should be 0)={batch_false_miss_albedo}",
                    out.h * out.w
                );
                std::io::stderr().flush().ok();
                if batch_nohit_px == 0 {
                    eprintln!("[{run_tag}] STARTUP ABORT: CURE 5's mask matched ZERO pixels on the first training batch — a cure that fires zero times is a configuration bug, never a silent no-op. Check nohit_albedo_sq/GAIA_V9I_NOHIT_ALBEDO_SQ and the albedo AOV field.");
                    std::io::stderr().flush().ok();
                    std::process::exit(1);
                }
            }
        ...
        println!(
            "[{run_tag}] epoch {}/{} n2n_mse={:.6} lr={:.6} pool_yaws=[{yaw_a1:.1},{yaw_a2:.1},{yaw_b:.1}] pool_ms={pool_ms:.0} hist_ms={hist_ms:.0} batch_ms={batch_ms:.0} nohit_px={epoch_nohit_px}/{epoch_total_px}({nohit_pct:.2}%) nohit_effective={epoch_nohit_effective} ({:.1}s)",
