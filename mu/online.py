...
from field_env import FieldEnv
...
def make_online_step(net, opt):
    ...
    select_action_fn = mx.compile(_select)
    ...
    def loss_fn(net, obs, next_obs, action, reward, onpolicy_mask):
        ...
        loss_policy = -mx.sum(onpolicy_mask * advantage * logp_taken) / n_onpolicy
...
def run(steps=250, seed=0, lr=1e-3, eps=0.2, log_path=None, quiet=False, episode_len=30,
    ...
    for step in range(steps):
        ...
        next_obs_np, reward = env.step(action)  # world sim tick, off-graph
