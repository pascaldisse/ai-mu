...
## arc3 — 3D render (RENDER法) + 4D block + MCTS verdict (2026-08-01)

段1 `packages/field/src/surface.rs` + bin `field-view3`. RENDER法: a field
render is ALWAYS 3D. Heightfield ray-march (surface skin) and emissive volume
mode; Lambert + Blinn spec; torus-periodic bilinear; one hue, luminance carries
geometry (NO colormap). `plane::encode_png` (grayscale) is now labelled the
DEBUG instrument, never a product path.
- G5 render determinism: two identical runs -> render_digest 0xf5cbb484a537d4d4,
  png sha ecbfba8e830b7acbcd0f425e48c40db864421464 (identical) PASS
- volume mode differs (0x5cc4467849b9b21); seed 7 moves field AND render digest PASS
- visual check: wave rings stand up as ridges/craters incl. wrap-around copies
  (torus) — geometry, not a heatmap.

段2 4D BLOCK (主令 2026-08-01) in `mu-serve`: the trajectory is a 4D block held
IMPLICITLY — seed + journal ops ARE the compression; state(t) = replay of the
first t ops. No frame is ever stored.
- `VIEW3 <path> [volume] [at k]` renders journal position k by REPLAY.
- `WHEN` -> 24B: u64 step_index | u64 journal_pos | f32 energy | f32 absmax.
  Speak-back can answer "when" because the observer sits at a journal POSITION
  on the entropy axis.
- G6 block consistency: `VIEW3 at |ops|` == live render, bit-identical
  (sha 2d2554a0…, digest 0x379273fb11699970); k=4 differs (0x3b2aafed4ac8f13e) PASS
- observer axis measured: energy 2.577 (step 5) -> 5.941 (25) -> 6.534 (45) ->
  6.839 (65). UNVERIFIED as an entropy claim: this run PUMPS energy in every
  tick (a poke per action), so monotonicity here is injection, not thermalisation.

段3 MCTS復帰 = **否 (NO, at this stage)**. Cost is affordable, behaviour is worse.
- cost @load 76 (contaminated but comparative, same session): amortized 0-sim
  1.414ms mean / 2.850 p99 · plan sims=8 1.919/4.110 · sims=16 2.252/6.751 ·
  sims=32 2.278/4.023 · sims=16 depth=2 2.627 · depth=4 3.542. Planning is
  latent-only MLP batching: +0.86ms over amortized at sims=32.
- behaviour, 600 ticks, same seed/eps: amortized act2 per 150 = 29/96/104/42,
  raw reward 0.159 -> 0.386. plan sims=16 act2 = 26/19/18/24 (chance = 18.75),
  reward 0.121 -> 0.121 (flat). Planning ACTED, learned nothing.
- 死 plan() on the acting path with the arc2 loss: reason — actions then come
  from softmax(Q/temp) of an UNTRAINED dynamics head, while the gradient is a
  REINFORCE term on the policy head's logp of that action → off-policy
  mismatch; the policy head gets no coherent signal and search keeps
  overriding it. The MuZero-correct repair (NOT done here, next lane): replace
  the REINFORCE policy term with cross-entropy distillation of plan()'s
  policy_target into the policy head, then re-run this A/B.

### UNVERIFIED (arc3)
- all timings still under load 12-76 — G1 vs the 120fps floor stays open.
- render cost per frame not benchmarked (480x480 CPU ray-march, off the tick path).
- entropy-axis interpretation of `WHEN` energy (see 段2).
- volume mode has no physical calibration — density/emission are display choices.
