...
### ★★ 07-27 EVE — PASCAL'S EYE REOPENS THE WAR: THE LIGHT WAS NEVER REAL
His live read at spawn: mirrors partial+dark · orbs cast NO light · glitchy
halos. Diagnosis (v9body 326129db, all evidence committed):
· INTEGRATOR HAS NO EMISSIVE NEE — only sun shadow rays (integrator.wgsl
  :477-486) + hit-emission (:474-475); emitters light NOTHING at any spp
  (bit-identical 512→2048 proven). "Not real raytracing" = literally true.
· MIRRORS miss delta branch: scene roughness 0.02-0.05 > MIRROR_ROUGHNESS
  1e-3 (:107) → stochastic GGX @ spp=2 = noise → diffuse-trained body
  collapses to dark. Truth mirrors fine at 512spp.
· HALOS = camera-only reprojection (rdirect_gather_split.wgsl:330-341),
  zero object-velocity; offline tracer shows crisp orbs. Object mv = the
  missing piece (DLSS-4 transfer-list item).
· ARTIFACT #9: rdirect_reference.rs dropped ALL dynamics from every
  "ground truth" ever made (static partition only) — fixed, default ON
  (GAIA_REF_INCLUDE_DYNAMICS + GAIA_REF_EYE/YAW/PITCH pose envs).
· ARTIFACT #8: entire bar/probe campaign judged ONE empty-harbor pose —
  no mirror/emitter/motion ever in the exam. Exam must move to spawn.
· CAMPAIGN REDEFINED (in-flight lane ms3g95sg): STAGE1 emissive NEE+MIS
  in integrator · STAGE2 GAIA_MIRROR_ROUGHNESS_MAX param → then object
  motion vectors → retrain body on real light at hard poses → re-exam at
  spawn → THEN ordeal/stamp talk. Old bar debate MOOT until light is real.
· v9cap verdict stands: capacity exhausted (0.002/2× params, sparkle
  ignites at 327k under mix .85); ep60 champion of the OLD exam only.
· Speed sealed: 14.53ms median all-levers (a717bab9+6fe24b22+44a2429).

### † 07-25 COMPACT ANCHOR (superseded 07-27)
CONTEXT: compact mid-afternoon 07-25. Live state:
· ★ v9y RUNNING (PID ~13004, mix=0.85, 600 ep, scratch/v9y-train.log,
  detached — survives everything): EQUILIBRIUM FOUND — sparkle crawls
  6.5→13.0 over ep149-299 (UNDER bar), drive FLAT 0.810 since ep199 (first
  config ever), resid PLATEAU 0.0465 since ep224 [source: v9y-train.log
  PROBE lines]. Ends ~16:30 local. v9x (mix=0.5) verdict: dial monotone —
  sparkle returns 55.3@399, resid 0.0421 best-ever [source: v9x commit].
· RULING IN FLIGHT (Pascal asked "any point training more?" — answered):
  epochs won't close 0.0465→0.035; levers = (1) capacity after SPEED
  RE-AUDIT (body [24,40,64], bridge 15.3ms vs 16.67 budget — thin; tracer
  fast, ms may hide in pipeline), (2) warm-restart schedule (cheap), (3)
  ★ PASCAL'S EYE FIRST: render v9y frame vs 512spp truth on run end, his
  verdict decides if 0.0465 is visible or numerology. EYE BEFORE LEVERS.
· Small-res val/mirror monitors flare late (325/542 sparkle) while bar-res
  stays legal — v9w2 signature too; eval frame decides artifact vs real.
· Eval tooling: examples/rdirect_v9w2_triptych.rs (byte-faithful compose
  proof pattern — reproduce PROBE numbers exactly before trusting a frame;
  adapt for v9y: V9V_INPUT_CLAMP=0.10 at eval, caps OFF, teacher 512spp).
· WINDOW: down by design. v9wire has motion-fixed wiring (55f17b31: mv
  channels were dead weights — zeroed live; /scry blit bug fixed). Eye-test
  relaunch only after motion-clean + a body worth showing.
· Whole v9k..v9y causal chain sealed below (07-24 + 07-23 sections):
  falsifications — noisy-teacher (v9u), loss-side, output-caps,
  flag-channel; PROVEN — 512spp truth 1.7s/frame (rdirect_reference.rs),
  input-clamp+motion starve 3× (v9v), display loss kills drive (v9w2
  9.8@299 falling), mix dial monotone (v9x/v9y).
· Env surface (all IRON default-off): V9_TEACHER_SPP · V9V_INPUT_CLAMP ·
  V9V_MOTION · V9W_LOSS_DOMAIN · V9X_MIX · V9_EPOCHS · V9_SPARKLE_ABORT ·
  v9n/o/p/q gates. Launch pattern + judged pose unchanged.
· OTHER: Handwerker-AI courtship + Charles → memory USER.md · gaia-daemon
  max-thinking LIVE · no other subagents at compact.
· NEXT IN ORDER: v9y end → eval frame vs truth → PASCAL'S EYE → then
  capacity/schedule/ordeal per his verdict.
· ★★ 15:24-16:02 UPDATE — IT WORKS (Pascal, in the window, his words):
  v9y ended wall-capped ~ep432; trainer best-scorer fooled by small-res
  flare → BEST saved = ep60 (sparkle 0.0, resid 0.0527, PROBE-matched);
  ep~300 sweet-zone body NEVER SAVED — checkpoint-cadence fix owed. ep60
  frames vs truth: cleanest body ever — no mottle, no fireflies; gaps =
  dashed waterline glints + dim windows. RESID/EYE DIVERGE (0.0527 looks
  better than v9w2's 0.0473) — metric ≠ eye, his eye rules. v9wire
  da33bf54: live gather had NO input clamp — ported to WGSL (median-of-4+
  delta, domain-proof vs trainer, IRON default-off), motion-verified 4
  captures + windowed relaunch d11d39b3. WINDOW LIVE on his screen
  (v9y-best, clamp 0.10, hitgate, ~19fps — un-fused CPU compose is the
  known debt; bridge 11.6ms). PENDING HIS WORD: water as-is → stamp path
  · brighter → capacity round. [source: v9y-train.log, 646bd9c4/1dc05cda]
· ★★ 07-26 ENDGAME STATE: v9z bar-best ep324 rendered — wall MOTTLE
  (disease as texture) → ep60 v9y-best stays CHAMPION by eye. SPEED WAR
  WON on v9 path: GPU-fuse 44a2429 + net-async 6fe24b22 + async-trace/
  trace-reuse a717bab9 (composite dispatch = dead code for v9, deleted;
  E+D≠composite ULP-filed honestly) → ALL-LEVERS TOTAL 14.53ms MEDIAN
  (p95 19.6) under 16.67 law [source: scratch/alllevers.log]. ORDEAL RAN
  HONESTLY on ep60: FAIL resid_still 0.0568 vs 0.035 → NO STAMP (law
  holds; stamp mechanism = real_image_ordeal auto-stamp on PASS,
  rdirect.rs:1734/1741/1754; REAL-OR-BLACK verified both ways 1530cf0a).
  /scry ungated dev-trace door FLAGGED (pre-existing capture-honesty gap,
  unfixed). → CAPACITY ROUND LAUNCHED (v9cap: GAIA_V9_WIDTHS env, wider
  body affordable now — net_gpu 4.61ms, headroom to ~7ms; bar-legal stamp
  is the goal). Freeze incident 08:17: GPU load + foreign java proc —
  machine-politeness orders standing (one server, nice builds).
· GAIA-WORLD golem-senses branch 14276511 (engine repo): occlusion tags
  (collider boxes gate eyes = body law) + curated alarm channel
  (/events?kind=alarm survives presence flood), 7/7 tests + live play
  proof; merge blocked on other lane's uncommitted rust-port work.
  ChatGPT pool capped til ~07-27 — all lanes route sonnet.

### † 07-24 GROUND-TRUTH REVOLUTION + SPARKLE-DRIVE KILL (dated history)
